...
def extract_text_from_json(data):
    ...
    if isinstance(data, list):
        return [
            item["text"] for item in data if isinstance(item, dict) and "text" in item
        ]
    return []
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: python3 scripts/compare_evaluations.py <path_to_pdf> <ground_truth_json>"
        )
        sys.exit(1)
    ...
    conv_off = DocChunker(pictures=False).converter
    ...
    conv_on = DocChunker(pictures=True).converter
